...
const zhHans: HomeCopy = {
  ...
  description: 'ObjectOS 是 AI 所写业务应用的开源目标格式和运行时。你的 Agent 编写紧凑的 ObjectStack 元数据——一个完整 CRM 不到 2000 行，因此团队要审的 diff 小到真的审得完；运行时在你控制的数据库上执行权限、审批和审计。',
  eyebrow: 'AI 写应用的受治理运行时',
  hero: ['AI 写应用。', 'ObjectOS 让它可治理。'],
  heroLead: '把编码 Agent 指向 ObjectStack。它把模型、界面、流程、权限和工具写成紧凑元数据，而不是整套代码——一个完整 CRM 不到 2000 行。团队审的就是这么小的 diff，运行时对每一次调用执行治理。',
  primaryCta: '开始使用',
  secondaryCta: '了解工作原理',
  proof: [['开源协议', 'Apache 2.0，可自托管'], ['天生可审阅', '一个完整 CRM 约 1800 行'], ['运行时治理', '每次调用都过权限与审计']],
...
const en: HomeCopy = {
  ...
  description: 'ObjectOS is the open-source target format and runtime for AI-written business applications. Your agent writes compact ObjectStack metadata — a complete CRM is under 2,000 lines — so your team reviews a diff small enough to actually review, and the runtime enforces permissions, approvals, and audit on a database you control.',
  eyebrow: 'Governed runtime for AI-written apps',
  hero: ['AI writes the app.', 'ObjectOS keeps it governable.'],
  heroLead: 'Point your coding agent at ObjectStack. It writes models, UI, workflows, permissions, and tools as compact metadata, not a full codebase — a complete CRM is under 2,000 lines. Your team reviews a diff that small, and the runtime enforces governance on every call.',
  primaryCta: 'Get started',
  secondaryCta: 'See how it works',
  proof: [['Open protocol', 'Apache 2.0, self-hostable'], ['Reviewable by design', 'A whole CRM is ~1,800 lines'], ['Governed at runtime', 'Permissions and audit on every call']],
...
const ja: HomeCopy = {
  ...
  description: 'ObjectOS は AI が書く業務アプリのためのオープンソースのターゲット形式とランタイムです。エージェントはコンパクトな ObjectStack メタデータを書きます。完全な CRM でも 2,000 行未満なので、チームが実際にレビューしきれる大きさの diff で済み、ランタイムがあなたの管理下のデータベース上で権限、承認、監査を実行します。',
  eyebrow: 'AI が書くアプリのガバナンス実行基盤',
  hero: ['AI がアプリを書く。', 'ObjectOS が統制する。'],
  heroLead: 'コーディングエージェントを ObjectStack に向けます。モデル、UI、ワークフロー、権限、ツールをフルコードではなくコンパクトなメタデータとして書きます。完全な CRM でも 2,000 行未満。チームはその大きさの diff をレビューし、ランタイムが呼び出しごとに統制を実行します。',
  primaryCta: 'はじめる',
  secondaryCta: '仕組みを見る',
  proof: [['オープンプロトコル', 'Apache 2.0・セルフホスト可'], ['レビュー可能な設計', 'CRM 全体で約 1,800 行'], ['ランタイムで統制', '呼び出しごとに権限と監査']],
...
const de: HomeCopy = {
  ...
  description: 'ObjectOS ist das quelloffene Zielformat und die Runtime für von AI geschriebene Business-Anwendungen. Ihr Agent schreibt kompakte ObjectStack-Metadaten — ein vollständiges CRM hat unter 2.000 Zeilen, der Diff ist also klein genug, um ihn wirklich zu prüfen — und die Runtime erzwingt Rechte, Freigaben und Audit auf einer Datenbank unter Ihrer Kontrolle.',
  eyebrow: 'Kontrollierte Runtime für AI-geschriebene Apps',
  hero: ['AI schreibt die App.', 'ObjectOS hält sie kontrollierbar.'],
  heroLead: 'Richten Sie Ihren Coding-Agent auf ObjectStack. Er schreibt Modell, UI, Workflows, Rechte und Agent-Tools als kompakte Metadaten statt als Codebasis — ein vollständiges CRM hat unter 2.000 Zeilen. Ihr Team prüft einen Diff dieser Größe, und die Runtime erzwingt Governance bei jedem Aufruf.',
  primaryCta: 'Loslegen',
  secondaryCta: 'So funktioniert es',
  proof: [['Offenes Protokoll', 'Apache 2.0, selbst hostbar'], ['Prüfbar by design', 'Ein ganzes CRM: ~1.800 Zeilen'], ['Governance zur Laufzeit', 'Rechte und Audit bei jedem Aufruf']],
...
const es: HomeCopy = {
  ...
  description: 'ObjectOS es el formato objetivo y runtime open source para aplicaciones de negocio escritas por AI. Tu agente escribe metadatos ObjectStack compactos — un CRM completo tiene menos de 2.000 líneas —, así que tu equipo revisa un diff lo bastante pequeño para revisarlo de verdad, y el runtime aplica permisos, aprobaciones y auditoría sobre una base de datos que controlas.',
  eyebrow: 'Runtime gobernado para apps escritas por AI',
  hero: ['La AI escribe la app.', 'ObjectOS la mantiene gobernada.'],
  heroLead: 'Apunta tu agente de código a ObjectStack. Escribe modelo, UI, workflows, permisos y herramientas como metadatos compactos, no como una base de código completa: un CRM completo tiene menos de 2.000 líneas. Tu equipo revisa un diff de ese tamaño y el runtime aplica gobierno en cada llamada.',
  primaryCta: 'Empezar',
  secondaryCta: 'Cómo funciona',
  proof: [['Protocolo abierto', 'Apache 2.0, autohospedable'], ['Revisable por diseño', 'Un CRM entero: ~1.800 líneas'], ['Gobierno en runtime', 'Permisos y auditoría en cada llamada']],
...
const fr: HomeCopy = {
  ...
  description: 'ObjectOS est le format cible et le runtime open source pour les applications métier écrites par l’AI. Votre agent écrit des métadonnées ObjectStack compactes — un CRM complet fait moins de 2 000 lignes —, si bien que votre équipe relit un diff assez petit pour être vraiment relu, et le runtime applique permissions, validations et audit sur une base de données que vous contrôlez.',
  eyebrow: 'Runtime gouverné pour applications écrites par l’AI',
  hero: ['L’AI écrit l’application.', 'ObjectOS la garde gouvernable.'],
  heroLead: 'Pointez votre agent de code vers ObjectStack. Il écrit modèle, UI, workflows, permissions et outils sous forme de métadonnées compactes, pas comme une base de code complète : un CRM complet fait moins de 2 000 lignes. Votre équipe relit un diff de cette taille, et le runtime applique la gouvernance à chaque appel.',
  primaryCta: 'Commencer',
  secondaryCta: 'Voir comment ça marche',
  proof: [['Protocole ouvert', 'Apache 2.0, auto-hébergeable'], ['Relisible par conception', 'Un CRM entier : ~1 800 lignes'], ['Gouverné au runtime', 'Permissions et audit à chaque appel']],
...
const ko: HomeCopy = {
  ...
  description: 'ObjectOS는 AI가 작성한 비즈니스 애플리케이션을 위한 오픈소스 대상 형식이자 런타임입니다. 에이전트는 압축된 ObjectStack 메타데이터를 작성하며, 완전한 CRM도 2,000줄 미만이라 팀이 실제로 검토할 수 있는 크기의 diff로 끝납니다. 런타임은 당신이 제어하는 데이터베이스 위에서 권한, 승인, 감사를 강제합니다.',
  eyebrow: 'AI가 작성한 앱을 위한 거버넌스 런타임',
  hero: ['AI가 앱을 작성합니다.', 'ObjectOS가 거버넌스를 유지합니다.'],
  heroLead: '코딩 에이전트를 ObjectStack에 연결하세요. 모델, UI, 워크플로, 권한, 도구를 전체 코드베이스가 아니라 압축된 메타데이터로 작성합니다 — 완전한 CRM도 2,000줄 미만입니다. 팀은 그 크기의 diff를 검토하고, 런타임은 모든 호출에 거버넌스를 강제합니다.',
  primaryCta: '시작하기',
  secondaryCta: '작동 방식 보기',
  proof: [['오픈 프로토콜', 'Apache 2.0, 셀프 호스팅'], ['설계상 검토 가능', 'CRM 전체가 약 1,800줄'], ['런타임 거버넌스', '모든 호출에 권한과 감사']],
