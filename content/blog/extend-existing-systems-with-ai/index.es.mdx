...
**En resumen:** volver AI-native tus sistemas existentes no exige migrar ni reconstruir primero: conecta ObjectOS a la base de datos que ya operas y deja que los agentes entiendan datos reales bajo tus permisos. Conecta, no reconstruyas.
...
Este flujo funciona **hoy** con datasources, routing por objeto, pushdown de ObjectQL y la capa de agentes. La federación turn-key —importación de schema en un paso, binding a schemas externos y compuertas de seguridad para escritura— ya está entregada: [ADR-0015](https://github.com/objectstack-ai/objectstack/blob/main/docs/adr/0015-external-datasource-federation.md) está **Accepted** (backend, REST y CLI implementados) y [ADR-0062](https://github.com/objectstack-ai/objectstack/blob/main/docs/adr/0062-external-datasource-runtime.md) es la decisión canónica de su mitad de runtime.

`os datasource list-tables <name>` enumera las tablas remotas y `os datasource introspect <name> --table <table>` genera un borrador de objeto desde la tabla viva. Con `schemaMode: 'external'` la plataforma no ejecuta DDL contra esa base de datos, y una declaración que no coincide con las columnas reales hace fallar el arranque con un diff (`validation: { onMismatch: 'fail' }`). Escribir exige dos opt-ins en dos archivos: `external.allowWrites: true` en el datasource **y** `external.writable: true` en el objeto; si falta uno, se rechazan inserts, updates y deletes. Esas compuertas son de la federación: en una base de datos que ObjectStack gestiona, read-only sigue siendo un `GRANT` de base de datos. En ADR-0015 quedan pendientes la UI de Studio y drivers para más dialectos SQL.
