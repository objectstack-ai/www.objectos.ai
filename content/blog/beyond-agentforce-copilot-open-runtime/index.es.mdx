---
title: "Más allá de Agentforce y Copilot Studio: por qué algunas empresas eligen un runtime abierto y autoalojado"
author: ObjectStack Team
description: "Una empresa estuvo a punto de firmar Agentforce y, en el último momento, descubrió que no alcanzaba la mitad de los datos que estaban al otro lado del muro. Estas suites son óptimas dentro de su propio ecosistema, siempre que tu mundo entero esté dentro de un muro. Este artículo compara con honestidad las suites cerradas y el runtime abierto y autoalojado: cuándo elegir la suite, cuándo elegir lo abierto y por qué \"usar los dos\" suele ser la respuesta correcta."
...
La suite quiere que tu mundo se mude a su muro; el runtime abierto da por hecho que tu mundo es disperso de origen, y su trabajo es lograr que los sistemas existentes soporten la IA de forma nativa, sin migrar primero. Los datos de cliente de aquella empresa estaban la mitad en Salesforce y la otra mitad en un sistema propio. En ObjectOS, puede usar una sola definición para recoger ambos lados en un mismo "cliente":
...
export const Customer = ObjectSchema.create({
  ...
  label: 'Cliente',
  sources: [
    Source.salesforce('Account'),        // esta mitad, la de las oportunidades, está aquí
    Source.database('legacy.customers'),  // esta mitad, la de las entregas y los tickets, está aquí
  ],
