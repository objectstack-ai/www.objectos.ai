---
title: "Coste de AI Agents: comparar precio por acción y runtime autoalojado"
description: "El cobro por acción o por token parece flexible, pero cuanto más autónomo es un agent, más llamadas a herramientas produce. El coste debe verse junto a uso, salida de datos, lock-in y operación propia."
...
| Runtime autoalojado | Runtime de ObjectStack; ObjectOS para operación gestionada o despliegue privado | Crece con la infraestructura, básicamente desacoplado del uso | Buena, si el uso se duplica la curva apenas se mueve | Tu propia infraestructura |
...
La clave es una distinción: **ObjectStack aporta el motor de ejecución y el runtime abiertos para la definición de negocio; en ObjectOS pagas por la experiencia de operación en producción de esa misma app ObjectStack, no por un segundo motor ni por cada llamada al runtime.** Esa experiencia incluye Build y Ask, controles de equipo, despliegue gestionado o privado y soporte. De ahí:

- **El coste se desacopla del uso.** El runtime de ObjectStack ejecuta la definición de negocio en tu infraestructura, y por muchas veces que el agent invoque herramientas gobernadas el runtime no se cobra por invocación; el coste lo determina la infraestructura, es predecible y planificable. El precio de ObjectOS cubre la operación en producción de la app, no un contador de llamadas al runtime.
...
- **No hay bloqueo.** La definición de negocio se expresa como metadatos en tu repositorio bajo un protocolo abierto (Apache 2.0), *diff*-eables y migrables. En ObjectOS compras la operación en producción de la misma app ObjectStack abierta, no un runtime propietario del que no puedas salir.
