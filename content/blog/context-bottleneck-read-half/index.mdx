...
Snowflake opened Summit 26 in San Francisco on June 1, 2026 with a keynote from CEO Sridhar Ramaswamy. Databricks opened Data + AI Summit at Moscone on June 15 with a keynote from CEO Ali Ghodsi, organised around four words: context, control, cost, choice. Both arrived at the same position from the main stage — frontier models are no longer the scarce input; your business context is — and both shipped a product line named for it. Snowflake put a governed semantic layer inside its catalog and called the result Horizon Context. Databricks announced Genie Ontology, a continuously learned context layer feeding its agent products.

A note on sourcing, because it changes how much weight these sentences can carry: what follows characterises the framing of those keynotes and uses each vendor's own product names. It does not quote either keynote, because the vendors' own materials were not reachable from where this was written, and a paraphrase repeated from a conference recap is not a source. Anything below that depends on a specific claim is stated as *what the product does*, which is checkable in the vendors' documentation, rather than *what someone said on stage*, which is not checkable here.
...
Start with what actually happens on each path, because the industry priced them as though they were mirror images.
...
**One column has a yes in every row, and it is not either of the two you bought.** The context layer enforces none of the seven. The gateway enforces a version of two. All seven are ultimately supplied by the destination.
...
The related failure mode — an agent's tools written *beside* your business logic instead of *from* it, drifting apart in one direction — the action-tools piece named **the second write path**. Borrowed enforcement is the layer above it: even when nobody hand-writes a tool, routing writes through connectors means every guarantee is the destination's, and it is worth knowing which destinations have any.
...
Five properties, in roughly the order you would have to acquire them. They are all answerable by reading definitions, which is the point — a property you can only assess by running the system is not one you can review before shipping.
...
If a rule has to be declared to be enforced, and a write needs five of them, then the artifact that governs an agent's writes is not a semantic model of your data. It is a definition of your application: objects, permissions, and the operations that may be performed on them, with the preconditions and the confirmation policy and the audit attached to the operation itself.
