---
title: "Después de instalar MCP en diez mil servidores: lo que le falta al agent empresarial no es el protocolo, es una capa de herramientas gobernada"
description: "Envolver una base de datos como herramienta MCP basta para que un agent \"se conecte\" en una tarde. Y luego, encantado, exporta a una tabla los importes de los contratos de todos los clientes de la empresa, porque esa envoltura no tiene identidad. MCP llegó en un año a más de diez mil servidores; la batalla de la conexión ya está ganada. Lo verdaderamente escaso es la capa de herramientas que hay detrás del protocolo: la que fuerza permisos, deja rastro y no tienes que escribir a mano."
...
Pero el propio éxito fabricó la trampa. La conexión —que viene a ser el 80% de todo el trabajo de ingeniería— de repente se resuelve en horas. Y así, ese 20% verdaderamente difícil y verdaderamente importante (una vez conectado, si está bien hecho hacerlo, y si se contabiliza una vez hecho) se salta con especial facilidad como un "ya lo añadiré luego". Aquella tarde del principio nació justo así.
