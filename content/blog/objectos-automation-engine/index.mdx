...
By default a flow runs **as the user who triggered it**, so its reads and writes respect that user's [row-level security](/en/glossary/row-level-security/). Elevation is a declared choice (`runAs: 'system'`), not an accident of where the code runs — and a run that declares `user` but resolves no triggering user has its data operations refused rather than quietly falling back to elevated.
...
- **An engine does not fix a bad process.** Modelling a broken [approval chain](/en/glossary/approval-workflow/) in metadata gives you a legible broken approval chain. That is genuinely better than an illegible one, but it is not the same as an improvement.
...
That is the whole bet: AI writes the flow, a human reviews a [small diff](/en/glossary/reviewable-diff/), and the engine decides, waits, and leaves the receipt.
