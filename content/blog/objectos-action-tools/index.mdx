...
- The [permission model](/en/glossary/permission-model/) tightens. The tool still calls the service account.
...
**A multi-step business approval is not an action field.** `requiresConfirmation` is a human-in-the-loop pause on the call. A real [approval workflow](/en/glossary/approval-workflow/) — routed, delegated, escalating, with a record of who signed — is its own metadata, and it should stay that way.
