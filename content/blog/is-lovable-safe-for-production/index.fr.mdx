...
description: "Lovable permet à un non-développeur de livrer une app full-stack à partir d'une phrase, mais le contrôle d'accès finit enfoui dans du code généré qu'il ne sait pas lire. Un scan de 4 000+ apps en a trouvé 170+ qui fuitent des PII réelles. 'Ça marche' n'est pas 'c'est sûr'."
...
**TL;DR :** Lovable est le moyen le plus rapide pour un non-développeur de livrer une vraie app full-stack monétisable. Le danger est précis et démontrable : le contrôle d'accès qui décide qui peut lire les données de vos utilisateurs est généré sous forme de SQL de sécurité au niveau des lignes (RLS) Supabase — par une IA, pour quelqu'un qui, par définition, ne sait pas lire le SQL — et le motif généré le plus courant, `using (true)`, *ressemble* à une politique de sécurité tout en ne protégeant rien. Un filtre frontend `.eq('owner', user.id)` qui fait que la démo se comporte correctement est une **commodité de requête, pas une frontière de sécurité**. Un scan de 4 000+ apps Lovable a trouvé 2 000+ vulnérabilités, 400+ secrets exposés et **175 qui fuitent des PII réelles** ; CVE-2025-48757 a exposé 170+ apps par exactement cette classe d'erreur. « Ça marche » et « c'est sûr » sont des affirmations différentes, et la personne qui livre ne sait pas les distinguer.
...
`using (true)` signifie : *tout appelant authentifié peut lire toutes les lignes.* La politique **existe**. Elle est **active**. Et elle ne protège **rien**. L'app est sûre dans exactement une configuration — celle que le frontend se trouve demander — et grande ouverte à l'instant où quelqu'un demande autre chose. Ce n'est pas hypothétique ; c'est le mécanisme derrière CVE-2025-48757 et les 170+ apps qui ont fuité de vraies données personnelles.
...
## Le chiffre qui explique 170 fuites : l'asymétrie de vérification
...
Quand le coût de construction s'effondre à zéro mais que le coût de vérification reste à des heures d'expert, le comportement rationnel d'un non-expert est de **sauter la vérification** — parce qu'il ne peut pas la faire et que l'app *a l'air* terminée. Multipliez cela sur une population et vous n'obtenez pas quelques fuites malchanceuses ; vous obtenez 175 apps qui fuitent des PII réelles en un seul scan. Le chiffre de 170+ n'est pas un problème de qualité de Lovable. C'est ce que l'asymétrie de vérification produit à l'échelle, avec n'importe quel outil qui laisse des non-experts générer un contrôle d'accès qu'ils ne savent pas lire.
...
Nous ne battrons pas Lovable sur le temps-jusqu'à-la-première-démo, et pour des prototypes nous n'essaierions pas. L'affirmation est celle qui compte à l'instant où de vraies données sont en jeu : la sécurité d'une app construite par IA doit être *visible pour la personne qui en répond* — parce qu'une app dont vous ne pouvez pas voir la frontière n'est pas sûre, elle est seulement non testée en public, et 170 équipes ont déjà passé ce test à la dure.
