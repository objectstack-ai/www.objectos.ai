---
title: "Une fois MCP installé sur dix mille serveurs : ce qui manque aux agents d'entreprise, ce n'est pas le protocole, c'est une couche d'outils gouvernée"
description: "Emballer une base de données en outil MCP suffit pour qu'un agent se « branche » en un après-midi. Et il exportera de bon cœur le montant des contrats de tous les clients de l'entreprise dans un tableau — parce que cette couche d'emballage n'a pas d'identité. MCP s'est déployé sur des dizaines de milliers de serveurs en un an, la bataille de la connexion est gagnée ; ce qui est vraiment rare, c'est la couche d'outils derrière le protocole, qui impose les permissions, laisse une trace, et que vous n'avez pas à écrire à la main."
...
Reconnaissons d'abord l'événement majeur : MCP s'est déployé en un an sur **des dizaines de milliers de serveurs d'entreprise, avec 97 millions de téléchargements de SDK** ; Anthropic, OpenAI, Google, Microsoft, AWS s'y sont tous ralliés ; A2A a franchi 150 organisations et a rejoint la Linux Foundation. La manière dont un agent « se connecte aux outils et aux autres agents » est désormais standardisée.

Mais le succès lui-même a fabriqué un piège. La connexion — qui représente à peu près 80 % de la charge d'ingénierie — se règle soudain en quelques heures. Du coup, les 20 % vraiment difficiles et vraiment importants (une fois connecté, a-t-il le droit de faire, et l'action est-elle consignée) sont d'autant plus facilement esquivés comme « à compléter plus tard ». L'après-midi du début, c'est exactement de là qu'il vient.
