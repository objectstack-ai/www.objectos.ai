---
title: "Nachdem MCP auf zehntausend Servern installiert ist: Was Enterprise-Agents fehlt, ist nicht das Protokoll, sondern eine governte Werkzeugschicht"
description: "Eine Datenbank als MCP-Tool zu verpacken, lässt einen Agent an einem Nachmittag „andocken\". Dann exportiert er bereitwillig die Vertragsvolumina aller Kunden der Firma als Tabelle – weil jene Verpackung keine Identität hat. MCP ist binnen eines Jahres auf zehntausend Servern ausgerollt, der Kampf um die Anbindung ist gewonnen; wirklich knapp ist die Werkzeugschicht hinter dem Protokoll, die Berechtigungen erzwingt, Spuren hinterlässt und die Sie nicht von Hand schreiben müssen."
...
Erkennen wir zuerst diese große Sache an: MCP ist innerhalb eines Jahres auf **zehntausend Unternehmensservern, 97 Millionen SDK-Downloads** ausgerollt; Anthropic, OpenAI, Google, Microsoft, AWS sind allesamt dabei; A2A hat über 150 Organisationen überschritten und ist an die Linux Foundation gegangen. „Wie ein Agent ein Werkzeug anbindet, wie er einen anderen Agent anbindet" ist bereits standardisiert.
