---
title: "Les géants se ruent tous sur l'ontologie d'entreprise, mais toutes sont fermées — la couche sémantique ouverte sera-t-elle le TCP/IP de l'ère de l'IA ?"
description: "Le verrouillage d'un fournisseur unique est un vieux problème ; trois géants construisant simultanément des ontologies fermées en créent un nouveau, bien pire — votre « client » est découpé en trois et aucun agent ne peut plus répondre à l'échelle globale. Ce qui manque vraiment à cette couche, ce n'est pas « plus d'ouverture », mais une couche de définition neutre qui n'appartient à personne."
...
Il y a un an, « couche sémantique métier / Ontologie » était encore essentiellement le vocabulaire propre à Palantir. Cette année, Gartner, lors de son sommet Data & Analytics, a directement positionné la couche sémantique et les graphes de connaissances comme l'infrastructure de l'IA agentique ; les prévisions du marché des graphes de connaissances d'entreprise passent d'environ 1,9 milliard de dollars aujourd'hui à près de 10 milliards en 2032. Les acteurs sont tous là : Microsoft pousse Fabric IQ, Google met le graphe de connaissances d'entreprise sur la table, et Palantir vend Foundry tout en s'arrimant à la puissance de calcul de NVIDIA.

Pourquoi maintenant, précisément ? Parce que l'agent a crevé l'abcès. La décennie de pipelines de données ne s'est souciée que de « où coule la donnée », pas vraiment de « ce que cette donnée signifie » — de toute façon, l'humain rattrapait en bout de chaîne. L'agent n'a pas ce filet humain : face à des dizaines de tables et à des champs « client » aux définitions discordantes, soit il répond faux (comme pour le groupe H), soit il n'ose pas répondre. Ces chiffres qui crèvent les yeux dans le secteur — 88 % des pilotes d'agents n'atteignent jamais la production — meurent en grande partie sur cette couche manquante.
...
**Troisièmement, beaucoup d'entreprises sont effectivement « presque tout chez un seul ».** Si 80 % de votre activité tient dans un seul écosystème, alors « le meilleur de l'écosystème » est pour vous l'optimum au sens littéral, et les bénéfices de l'ouverture ne vous serviraient à rien.
...
## Les couches dont tout le monde dépend finissent toujours neutres
...
Supposons que Yuanfeng ait d'emblée bâti son « client » en une définition neutre : il aurait pu, avec une seule déclaration, rassembler en un même objet les données dispersées dans trois systèmes :
...
export const Customer = ObjectSchema.create({
  ...
  label: 'Client',
  sources: [
    Source.fabric('Account'),              // les commandes et les équipements sont ici
    Source.salesforce('Account'),          // les fiches commerciales issues de l'acquisition sont ici
    Source.database('eu.customers'),       // les données européennes isolées pour la conformité sont ici
  ],
...
os deploy --self-hosted   # la même définition tourne sur votre propre infrastructure
...
Une couche dont tout le monde dépend n'a jamais, dans l'histoire, appartenu durablement à un seul. Cette fois, aucune raison qu'il en aille autrement.
