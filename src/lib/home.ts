...
const zhHans: HomeCopy = {
  ...
  hero: ['AI 把整个应用建出来', '真实、可治理、归你所有。'],
  heroLead: 'ObjectOS 是开源、AI 原生的应用平台。描述你的业务，AI 就把一个真正的应用——数据模型、界面、流程、权限——建在一个你自己拥有的真实数据库上。自托管，或用我们的云。',
...
const en: HomeCopy = {
  ...
  hero: ['AI builds the whole application', 'real, governed, and yours.'],
  heroLead: 'ObjectOS is the open-source, AI-native app platform. Describe your business and AI builds a real application — data model, UI, workflows, and permissions — on a real database you own. Self-host it, or run it in our cloud.',
...
const ja: HomeCopy = {
  ...
  hero: ['AI がアプリ全体を構築', '本物で、統制され、あなたのもの。'],
  heroLead: 'ObjectOS はオープンソースの AI-native アプリ基盤です。業務を説明すれば、AI がデータモデル、画面、フロー、権限を備えた本物のアプリを、自分が所有するデータベース上に構築します。セルフホストでも、私たちのクラウドでも。',
...
const de: HomeCopy = {
  ...
  hero: ['AI baut die ganze Anwendung', 'echt, kontrolliert und Ihre.'],
  heroLead: 'ObjectOS ist die quelloffene, AI-native App-Plattform. Beschreiben Sie Ihr Geschäft und AI baut eine echte Anwendung — Datenmodell, Oberfläche, Prozesse und Berechtigungen — auf einer Datenbank, die Ihnen gehört. Selbst gehostet oder in unserer Cloud.',
...
const es: HomeCopy = {
  ...
  hero: ['La AI crea toda la aplicación', 'real, gobernada y tuya.'],
  heroLead: 'ObjectOS es la plataforma de apps AI-native y de código abierto. Describe tu negocio y la AI construye una aplicación real — modelo de datos, interfaz, flujos y permisos — sobre una base de datos que es tuya. Autohospédala o úsala en nuestra nube.',
...
const fr: HomeCopy = {
  ...
  hero: ['L’AI construit toute l’application', 'réelle, gouvernée, et à vous.'],
  heroLead: 'ObjectOS est la plateforme d’applications AI-native et open source. Décrivez votre métier et l’AI construit une vraie application — modèle de données, interface, processus et permissions — sur une base de données qui vous appartient. Auto-hébergez-la ou utilisez notre cloud.',
...
const ko: HomeCopy = {
  ...
  hero: ['AI가 애플리케이션 전체를 구축', '진짜이고, 거버넌스가 있고, 당신의 것.'],
  heroLead: 'ObjectOS는 오픈소스 AI-native 앱 플랫폼입니다. 비즈니스를 설명하면 AI가 데이터 모델, 화면, 워크플로, 권한을 갖춘 진짜 애플리케이션을 당신이 소유한 데이터베이스 위에 구축합니다. 셀프 호스팅하거나 우리 클라우드로.',
