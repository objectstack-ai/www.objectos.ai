...
**`allowDelete: false → true`.** No rule fires. Both values are valid; both are enforceable; the runtime will enforce whichever one you merge. Nothing in the metadata expresses whether sales representatives should be able to delete opportunities, because that is a business fact about your company. This single character is the entire third rung, and it is invisible to every one of the 42 rules.
