...
| 2 | `config.triggerType` is an array containing a `record-` token | `record_change` — routed there on purpose, so it is refused loudly at bind time instead of vanishing |
...
| Style | Fires | Misses |
| --- | --- | --- |
| Event-driven (`record_change`) | exactly when the fact changes | anything that becomes true without a write — every date-based rule |
| Scheduled (`schedule`) | on cadence, whether or not anything happened | a window the platform was down for; there is no replay |
| Sweep (`time_relative`) | once per matching record, per sweep day | records beyond the per-sweep cap, and matches outside the declared window |
| Inbound (`api`) | on the sender's schedule | nothing — but it may fire twice, so idempotency is yours to author |

Date-based rules belong to neither of the first two, which is precisely why the sweep exists as its own trigger type rather than as a documented pattern. When a platform lacks it, teams rebuild it badly, one contract-renewal cron job at a time.
...
| --- | --- | --- |
| `flow-trigger-unroutable` | a `record_change` flow whose start-node `triggerType` routes nowhere — off-grammar like `onCreate`, or missing entirely | error |
