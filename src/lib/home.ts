...
const zhHans: HomeCopy = {
  title: 'ObjectOS · 开源 AI 原生应用平台',
  description: 'ObjectOS 是开源、AI 原生的应用平台，用于构建真正的业务应用。描述你的需求，AI 就把数据模型、界面、流程和权限建出来——数据库归你所有。自托管，或用云。',
  eyebrow: 'AI 原生应用平台',
  hero: ['用 AI 搭建，', '用 AI Agent 问询和运行。'],
  heroLead: 'ObjectOS 是开源、AI 原生的应用平台。AI 把整个应用——数据模型、界面、流程、权限——建在一个你自己拥有的真实数据库上；应用跑起来后，AI Agent 还能在用户权限内对业务数据问询、分析并执行操作。自托管，或用我们的云。',
  primaryCta: '开始使用',
  secondaryCta: '了解工作原理',
  proof: [['开源', '自托管，数据归你'], ['AI 直建', '真应用，不只是表格'], ['内置治理', '权限与审计']],
  ...
  aiCopy: 'ObjectOS 把对象、字段、流程、权限和动作变成 AI 可读写的声明式元数据，并通过受控工具访问。开源版采用自带 AI（BYO-AI）：你的编码 Agent 以源码方式编写元数据，你像审阅小 diff 一样确认；任意 MCP 客户端即可问询数据。界面内的 Build 与 Ask 助手——在产品里用自然语言搭建、对数据问询——在云端与企业版提供。',
  aiItems: [
    { title: 'AI Builder', copy: '云端与企业版：在界面内用自然语言搭建——内置 Builder 生成对象、字段、视图、流程和权限，结构性变更进入审批。开源自托管时，由你自己的编码 Agent 以源码方式编写同样的元数据，你审阅 diff。' },
    { title: 'AI Ask', copy: '云端与企业版：在产品内对数据问询——查询业务对象、分析上下文，并在登录用户权限内触发动作。开源版则通过 MCP 用你自己的 AI 问询同样的对象。' },
    { title: 'Tools / MCP', copy: '各版本通用、自带 AI：@objectstack/mcp 把对象、动作和查询暴露为受控、受权限约束的工具，供 Claude、Cursor、任意 MCP 客户端或本地模型调用。' },
  ],
  aiLink: '查看 AI 安全模型',
  workflowKicker: '工作方式',
...
const en: HomeCopy = {
  ...
  description: 'ObjectOS is the open-source target format and runtime for AI-written business applications. Your agent generates ObjectStack metadata, humans review a small diff, and the runtime enforces permissions, approvals, and audit on a database you control.',
  eyebrow: 'Governed runtime for AI-written apps',
  hero: ['AI writes the app.', 'ObjectOS keeps it governable.'],
  heroLead: 'Point your coding agent at ObjectStack. It generates the data model, UI, workflows, permissions, and agent tools as reviewable metadata. ObjectOS runs that application on a real database you control, with permissions, approvals, and audit built into the runtime. Self-host it, or run it in our cloud.',
  primaryCta: 'Get started',
  secondaryCta: 'See how it works',
  proof: [['Open protocol', 'Apache 2.0, self-hostable'], ['Reviewable changes', 'Small metadata diffs'], ['Runtime governance', 'Permissions, approvals, audit']],
  ...
  positioningCopy: 'Enterprise AI does not need another rebuild project. It needs a way to connect legacy systems, new applications, and AI agents while keeping every change understandable, reviewable, and governed.',
  ...
  capabilities: [
    ...
    { title: 'Generate applications as metadata', copy: 'Turn a business request into objects, views, workflows, permissions, APIs, and agent tools your team can review.' },
  ...
  aiCopy: 'ObjectOS turns objects, fields, workflows, permissions, and actions into declarative metadata that agents can read and update through governed tools. In the open-source edition, you bring your own AI: a coding agent writes metadata as source files, you review the diff, and any MCP client can query your data. The in-app Build and Ask assistants run on Cloud and Enterprise.',
  aiItems: [
    { title: 'AI Builder', copy: 'Cloud & Enterprise: describe a change in natural language. The in-app Builder generates objects, fields, views, workflows, and permissions, then routes structural changes for approval. In open source, your coding agent writes the same metadata as a diff.' },
  ...
  workflowCopy: 'ObjectOS describes objects, relationships, permissions, workflows, and actions in unified metadata. Agents work from explicit rules instead of inferring business logic from scattered code.',
  ...
  closingCopy: 'Connect one existing system, define its key business objects, and launch the first governed AI-written application on real data.',
...
const ja: HomeCopy = {
  ...en,
  title: 'ObjectOS · オープンソースの AI-native アプリ基盤',
  description: 'ObjectOS は本物の業務アプリを作るためのオープンソース AI-native 基盤です。要件を説明すれば、AI がデータモデル、画面、フロー、権限を生成します。自分が所有するデータベース上で、セルフホストでもクラウドでも。',
  eyebrow: 'AI-native アプリ基盤',
  hero: ['AI で構築。', 'AI エージェントで運用。'],
  heroLead: 'ObjectOS はオープンソースの AI-native アプリ基盤です。AI がデータモデル、画面、フロー、権限を備えたアプリ全体を、自分が所有するデータベース上に構築。さらに AI エージェントがユーザー権限の範囲でデータを問い合わせ、分析し、操作を実行します。セルフホストでも、私たちのクラウドでも。',
  primaryCta: 'はじめる',
  secondaryCta: '仕組みを見る',
  proof: [['オープンソース', 'セルフホスト・データは自分のもの'], ['AI が構築', '表計算ではなく本物のアプリ'], ['ガバナンス内蔵', '権限と監査']],
  ...
  aiCopy: 'ObjectOS はオブジェクト、項目、フロー、権限、アクションを、AI が管理されたツールで読み書きできる宣言的メタデータにします。オープンソース版は BYO-AI：自分のコーディング Agent がメタデータをソースとして書き、差分としてレビューでき、任意の MCP クライアントがデータを問い合わせます。製品内の Build／Ask アシスタント（自然言語での構築とデータ問い合わせ）はクラウド／Enterprise で提供します。',
  aiItems: [
    { title: 'AI Builder', copy: 'クラウド／Enterprise：製品内で自然言語から構築——内蔵 Builder がオブジェクト、項目、ビュー、フロー、権限を生成し、構造変更は承認へ。オープンソースでは、自分のコーディング Agent が同じメタデータをソースとして書き、差分としてレビューします。' },
    { title: 'AI Ask', copy: 'クラウド／Enterprise：製品内でデータに問い合わせ——業務オブジェクトを検索し、文脈を分析し、ログインユーザーの権限内でアクションを実行します。オープンソースでは、同じオブジェクトを MCP 経由で自分の AI から問い合わせます。' },
    { title: 'Tools / MCP', copy: '全エディション共通・BYO-AI：@objectstack/mcp がオブジェクト、アクション、クエリを、権限で制約された管理ツールとして Claude、Cursor、任意の MCP クライアント、ローカルモデルに公開します。' },
  ],
...
const de: HomeCopy = {
  ...en,
  title: 'ObjectOS · Open-Source AI-native App-Plattform',
  description: 'ObjectOS ist die quelloffene, AI-native Plattform für echte Geschäftsanwendungen. Beschreiben Sie, was Sie brauchen, und AI baut Datenmodell, Oberfläche, Prozesse und Berechtigungen — auf einer Datenbank, die Ihnen gehört. Selbst gehostet oder in der Cloud.',
  eyebrow: 'AI-native App-Plattform',
  hero: ['Mit AI bauen.', 'Mit AI-Agenten betreiben.'],
  heroLead: 'ObjectOS ist die quelloffene, AI-native App-Plattform. AI baut die ganze Anwendung — Datenmodell, Oberfläche, Prozesse und Berechtigungen — und AI-Agenten fragen anschließend Ihre Daten ab und betreiben sie im Rahmen der Nutzerrechte. Auf einer Datenbank, die Ihnen gehört. Selbst gehostet oder in unserer Cloud.',
  primaryCta: 'Loslegen',
  secondaryCta: 'So funktioniert es',
  proof: [['Open Source', 'Selbst hosten, Ihre Daten'], ['Von AI gebaut', 'Echte Apps, keine Tabellen'], ['Kontrolliert', 'Rechte und Audit integriert']],
  ...
  aiCopy: 'ObjectOS macht Objekte, Felder, Prozesse, Berechtigungen und Aktionen zu deklarativen Metadaten, die AI über kontrollierte Tools liest und ändert. Die Open-Source-Edition ist Bring-your-own-AI: Ihr Coding-Agent schreibt Metadaten als Quellcode, den Sie als Diff prüfen, und jeder MCP-Client fragt Ihre Daten ab. Die In-App-Assistenten Build und Ask — Bauen in natürlicher Sprache und Datenchat im Produkt — laufen auf Cloud und Enterprise.',
  aiItems: [
    { title: 'AI Builder', copy: 'Cloud & Enterprise: Bauen in natürlicher Sprache — der In-App-Builder generiert Objekte, Felder, Views, Workflows und Berechtigungen, Strukturänderungen laufen in die Freigabe. In der Open-Source-Edition schreibt Ihr eigener Coding-Agent dieselben Metadaten als Quellcode, den Sie als Diff prüfen.' },
    { title: 'AI Ask', copy: 'Cloud & Enterprise: Fragen Sie Ihre Daten im Produkt — Geschäftsobjekte abfragen, Kontext analysieren und Aktionen innerhalb der Rechte des angemeldeten Nutzers auslösen. In der Open-Source-Edition fragen Sie dieselben Objekte über MCP mit Ihrer eigenen AI ab.' },
    { title: 'Tools / MCP', copy: 'Jede Edition, Bring-your-own-AI: @objectstack/mcp stellt Objekte, Aktionen und Abfragen als kontrollierte, rechtebeschränkte Tools für Claude, Cursor, jeden MCP-Client oder ein lokales Modell bereit.' },
  ],
...
const es: HomeCopy = {
  ...en,
  title: 'ObjectOS · Plataforma de apps AI-native de código abierto',
  description: 'ObjectOS es la plataforma AI-native y de código abierto para crear aplicaciones de negocio reales. Describe lo que necesitas y la AI genera el modelo de datos, la interfaz, los flujos y los permisos — sobre una base de datos que es tuya. Autohospédala o úsala en la nube.',
  eyebrow: 'Plataforma de apps AI-native',
  hero: ['Constrúyelo con AI.', 'Opéralo con agentes de AI.'],
  heroLead: 'ObjectOS es la plataforma de apps AI-native y de código abierto. La AI construye toda la aplicación — modelo de datos, interfaz, flujos y permisos — y luego los agentes de AI consultan tus datos y la operan dentro de los permisos de cada usuario. Sobre una base de datos que es tuya. Autohospédala o úsala en nuestra nube.',
  primaryCta: 'Empezar',
  secondaryCta: 'Cómo funciona',
  proof: [['Código abierto', 'Autohospédalo, tus datos'], ['Hecho con AI', 'Apps reales, no hojas de cálculo'], ['Gobernado', 'Permisos y auditoría incluidos']],
  ...
  aiCopy: 'ObjectOS convierte objetos, campos, procesos, permisos y acciones en metadatos declarativos que la AI lee y cambia mediante herramientas gobernadas. La edición open source es trae-tu-propia-AI: tu agente de código escribe los metadatos como fuente que revisas como un diff, y cualquier cliente MCP consulta tus datos. Los asistentes integrados Build y Ask — construir en lenguaje natural y chatear con los datos dentro del producto — están en Cloud y Enterprise.',
  aiItems: [
    { title: 'AI Builder', copy: 'Cloud y Enterprise: construye en lenguaje natural — el Builder integrado genera objetos, campos, vistas, flujos y permisos, con los cambios estructurales enviados a aprobación. En open source, tu propio agente de código escribe los mismos metadatos como fuente que revisas como un diff.' },
    { title: 'AI Ask', copy: 'Cloud y Enterprise: consulta tus datos dentro del producto — consulta objetos de negocio, analiza contexto y ejecuta acciones dentro de los permisos del usuario conectado. En open source, consulta los mismos objetos mediante MCP con tu propia AI.' },
    { title: 'Tools / MCP', copy: 'Cualquier edición, trae tu propia AI: @objectstack/mcp expone objetos, acciones y consultas como herramientas gobernadas y limitadas por permisos para Claude, Cursor, cualquier cliente MCP o un modelo local.' },
  ],
...
const fr: HomeCopy = {
  ...en,
  title: 'ObjectOS · Plateforme d’applications AI-native open source',
  description: 'ObjectOS est la plateforme d’applications AI-native et open source pour créer de vraies applications métier. Décrivez votre besoin et l’AI génère le modèle de données, l’interface, les processus et les permissions — sur une base de données qui vous appartient. Auto-hébergée ou dans le cloud.',
  eyebrow: 'Plateforme d’applications AI-native',
  hero: ['Construisez avec l’AI.', 'Exploitez avec des agents AI.'],
  heroLead: 'ObjectOS est la plateforme d’applications AI-native et open source. L’AI construit toute l’application — modèle de données, interface, processus et permissions — puis des agents AI interrogent vos données et l’exploitent dans le cadre des permissions de chaque utilisateur. Sur une base de données qui vous appartient. Auto-hébergez-la ou utilisez notre cloud.',
  primaryCta: 'Commencer',
  secondaryCta: 'Voir comment ça marche',
  proof: [['Open source', 'Auto-hébergé, vos données'], ['Construit par l’AI', 'De vraies apps, pas des tableurs'], ['Gouverné', 'Permissions et audit intégrés']],
  ...
  aiCopy: 'ObjectOS transforme objets, champs, processus, permissions et actions en métadonnées déclaratives que l’AI lit et modifie via des outils gouvernés. L’édition open source est « apportez votre propre AI » : votre agent de code écrit les métadonnées sous forme de source que vous relisez comme un diff, et n’importe quel client MCP interroge vos données. Les assistants intégrés Build et Ask — construction en langage naturel et chat sur les données dans le produit — sont disponibles sur Cloud et Enterprise.',
  aiItems: [
    { title: 'AI Builder', copy: 'Cloud et Enterprise : construisez en langage naturel — le Builder intégré génère objets, champs, vues, workflows et permissions, les changements structurants passant en validation. En open source, votre propre agent de code écrit les mêmes métadonnées sous forme de source que vous relisez comme un diff.' },
    { title: 'AI Ask', copy: 'Cloud et Enterprise : interrogez vos données dans le produit — interrogez les objets métier, analysez le contexte et déclenchez des actions dans les permissions de l’utilisateur connecté. En open source, interrogez les mêmes objets via MCP avec votre propre AI.' },
    { title: 'Tools / MCP', copy: 'Toutes éditions, apportez votre propre AI : @objectstack/mcp expose objets, actions et requêtes comme outils gouvernés et limités par les permissions pour Claude, Cursor, tout client MCP ou un modèle local.' },
  ],
...
const ko: HomeCopy = {
  ...en,
  title: 'ObjectOS · 오픈소스 AI-native 앱 플랫폼',
  description: 'ObjectOS는 진짜 비즈니스 애플리케이션을 만드는 오픈소스 AI-native 플랫폼입니다. 필요한 것을 설명하면 AI가 데이터 모델, 화면, 워크플로, 권한을 생성합니다 — 당신이 소유한 데이터베이스 위에서. 셀프 호스팅하거나 클라우드로.',
  eyebrow: 'AI-native 앱 플랫폼',
  hero: ['AI로 만들고,', 'AI 에이전트로 운영하세요.'],
  heroLead: 'ObjectOS는 오픈소스 AI-native 앱 플랫폼입니다. AI가 데이터 모델, 화면, 워크플로, 권한을 갖춘 애플리케이션 전체를 당신이 소유한 데이터베이스 위에 구축하고, 이후 AI 에이전트가 사용자 권한 범위에서 데이터를 질의·분석하고 작업을 실행합니다. 셀프 호스팅하거나 우리 클라우드로.',
  primaryCta: '시작하기',
  secondaryCta: '작동 방식 보기',
  proof: [['오픈소스', '셀프 호스팅, 데이터는 당신 것'], ['AI가 구축', '스프레드시트가 아닌 진짜 앱'], ['거버넌스 내장', '권한과 감사']],
  ...
  aiCopy: 'ObjectOS는 객체, 필드, 프로세스, 권한, 작업을 AI가 관리되는 도구로 읽고 변경할 수 있는 선언형 메타데이터로 만듭니다. 오픈소스 에디션은 BYO-AI입니다: 당신의 코딩 에이전트가 메타데이터를 소스로 작성하면 diff처럼 검토하고, 모든 MCP 클라이언트가 데이터를 조회합니다. 제품 내 Build·Ask 어시스턴트 — 자연어 구축과 데이터 대화 — 는 Cloud와 Enterprise에서 제공됩니다.',
  aiItems: [
    { title: 'AI Builder', copy: 'Cloud · Enterprise: 제품 안에서 자연어로 구축 — 내장 Builder가 객체, 필드, 뷰, 워크플로, 권한을 생성하고 구조 변경은 승인으로 보냅니다. 오픈소스에서는 당신의 코딩 에이전트가 같은 메타데이터를 소스로 작성하고 diff로 검토합니다.' },
    { title: 'AI Ask', copy: 'Cloud · Enterprise: 제품 안에서 데이터에 질의 — 비즈니스 객체를 조회하고 맥락을 분석하며 로그인 사용자 권한 안에서 작업을 실행합니다. 오픈소스에서는 같은 객체를 MCP로 당신의 AI에서 조회합니다.' },
    { title: 'Tools / MCP', copy: '모든 에디션, BYO-AI: @objectstack/mcp가 객체, 작업, 쿼리를 권한으로 제한된 관리 도구로 Claude, Cursor, 모든 MCP 클라이언트 또는 로컬 모델에 노출합니다.' },
  ],
  aiLink: 'AI 보안 모델 보기',
