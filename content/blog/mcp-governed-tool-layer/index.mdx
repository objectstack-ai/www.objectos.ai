...
All five are questions about **the call**. There is a sixth that this layer cannot answer for you, because it is a property of wherever the call lands: what does the destination itself enforce on the write? [Every context fix solves only the read half](/en/blog/context-bottleneck-read-half/) works through that gap — a governed tool layer plus a context layer still leaves the operation's own rules being borrowed from whatever sits at the far end.
...
The open ObjectStack runtime then **automatically** projects this into a set of governed tools the agent can call: query refunds and create refunds, each with permission checks built in, executing under the logged-in user's identity, and landing in a unified audit ledger. "Over-limit refunds go through approval" becomes a process attached to the object, automatically pausing for a signature when the tool reaches it. None of this has to be hand-written in tool code. Change one permission, and all related tools change in one place.
...
Projection is no longer a minority position: the major ontology platforms now publish their object and action types straight onto an MCP surface, and the rule they use has a shape worth knowing before you plan around it — [what Ontology MCP actually projects](/en/blog/ontology-mcp-agent-tools/) covers it, including why the read side and the write side end up needing different kinds of gate.
