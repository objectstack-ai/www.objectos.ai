...
**The short version:** whether an agent *should* respect permissions is settled. *Where the check runs* is not — and that is the part that decides whether the guarantee is real. There are four enforcement points: the query, the field, the tool gate, and the action body. Only the first three sit before data reaches the model. Anything enforced after that is a **paper permission** — metadata that reads as access control and enforces nothing. The useful question for a platform is not "does it have a [permission model](/en/glossary/permission-model/)." It is "can it prove a given permission is not made of paper."
...
A [paper permission](/en/glossary/declared-vs-enforced/) is any rule that a human reads as a restriction and that the runtime never applies. It is not a hypothetical category — it has at least four common species:
...
In ObjectStack, [row-level security](/en/glossary/row-level-security/) is authorable metadata: `permissions[].rowLevelSecurity[].using` for reads and `.check` for writes. Each is an expression the engine lowers into a filter condition and pushes into the query.
...
The same asymmetry shows up one level out, and for the same reason. Points 1 and 2 are read-side checks that generalise beautifully because a read is a predicate over data; the moment the operation is a verb, enforcement stops being a filter and starts being whatever the executing system happens to require. [Every context fix solves only the read half](/en/blog/context-bottleneck-read-half/) follows that consequence across a whole agent architecture, where the write's rules end up borrowed from the destination rather than declared anywhere you can inspect.
