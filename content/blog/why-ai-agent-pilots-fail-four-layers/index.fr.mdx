...
description: "Un projet a fait un tabac le jour de la démo et a été discrètement arrêté quatre mois plus tard. Ce qui l'a tué, ce n'est pas le modèle, c'est une question du service juridique. 88 % des pilotes d'agents s'arrêtent au PoC, 95 % n'obtiennent aucun retour — changer pour un modèle plus puissant ne sert presque à rien, car 80 % du travail relève de la donnée et de la gouvernance. Cet article décortique sur quelles quatre couches les pilotes meurent, et compare avec ce que les 12 % parvenus en production ont fait de juste."
