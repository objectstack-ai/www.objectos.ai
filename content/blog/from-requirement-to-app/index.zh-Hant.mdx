...
title: 從一句話需求到可執行應用：用裝置報修看 ObjectStack 後設資料
description: 用一個裝置報修場景拆開 AI Builder 的生成過程：物件、欄位、關係、檢視、許可權、動作、流程、API 和 Agent 工具如何由同一份 ObjectStack 後設資料驅動。
...
**先給結論**：用一個裝置報修場景走一遍——物件、欄位、檢視、許可權、動作、流程、API 和 Agent 工具都來自同一份後設資料；這就是“生成可執行應用”和“一次性甩出程式碼”的根本區別。

你對 AI Builder 說一句話：

> 幫我做一個裝置報修系統：員工能提交故障，系統能關聯裝置、分派工程師、跟蹤狀態、計算停機時間，高優先順序自動派單，關閉工單前要填寫維修結果和成本。

幾分鐘後，一個可執行的應用出現了。它不只是有頁面，也有物件、欄位、關係、許可權、檢視、流程、動作、API 和 Agent 工具。

這篇不泛講“AI 生成應用”，而是用這個具體業務場景拆開看：ObjectStack 後設資料到底生成了什麼，為什麼它比一次性程式碼生成更適合企業應用。
