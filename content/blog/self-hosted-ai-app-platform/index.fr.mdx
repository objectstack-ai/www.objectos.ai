...
**En bref :** ce que vous devriez auto-héberger en premier n’est pas nécessairement le modèle — c’est le runtime qui détient les objets, les permissions, les outils, les validations et l’audit. Les modèles peuvent venir de l’extérieur ; la couche qui décide qui peut lire quoi, faire quoi et quelles preuves sont conservées appartient à votre propre infrastructure.
...
![Limite du runtime AI auto-hébergé](./self-hosted-runtime-en.svg)
...
L’essentiel est que le modèle ne se connecte pas directement aux bases et ne possède pas les identifiants métier. Il reçoit seulement le contexte minimal approuvé via l'ObjectStack Runtime open source. ObjectOS est la plateforme commerciale de production et l'expérience opérée optionnelles pour la même application ObjectStack ; il ne remplace pas cette frontière du runtime.
...
## ObjectStack Runtime et la couche d'exploitation ObjectOS optionnelle

L'ObjectStack Runtime open source ne commence pas par déplacer les données vers un nouveau cloud.

Il agit comme une couche opérationnelle auto-hébergée dans l’infrastructure de l’entreprise : connecter les systèmes existants, les modéliser comme objets, unifier permissions, workflows, API, audit et outils d’agent.

ObjectOS est la plateforme commerciale de production et l'expérience opérée optionnelles pour la même application ObjectStack. Il ajoute construction AI dans le navigateur, déploiement, support et exploitation sans modifier la définition ouverte ni l'ObjectStack Runtime sous-jacents.
