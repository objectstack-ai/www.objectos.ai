---
title: "El lock-in de Power Platform: solo Azure, sin self-host y tus datos rehenes en Dataverse"
description: "La gravedad de Power Platform es real: ya está en tu tenant. Pero no puedes self-hostearlo, una exportación se lleva los metadatos y no tus datos, y las apps creadas por IA quedan como config gestionada opaca."
...
El punto de partida honesto es que la gravedad es real y racional. Power Platform reporta ~56 millones de usuarios activos mensuales; Microsoft dice que más del 80 % de las Fortune 500 han creado agentes con Copilot Studio. Si tu empresa funciona sobre Microsoft 365, la plataforma está *justo ahí*: una identidad de Entra, ganchos nativos hacia Teams, SharePoint y Dynamics, una factura, una relación de soporte. Para una empresa estandarizada en Microsoft, elegirla suele ser la decisión correcta, y este artículo no va a fingir lo contrario.
...
Para una clase de comprador, el modelo de coste y la asimetría de salida son irrelevantes, porque un solo hecho termina la evaluación: **no existe un Power Platform self-hosted, on-prem ni air-gapped.** Si eres un banco bajo reglas de residencia, una carga de defensa o gobierno que debe estar air-gapped, o un sistema sanitario con requisitos de soberanía, no puedes poner el *runtime* donde tus reguladores lo exigen: ni por un precio, ni en un roadmap. Todo lo demás en este artículo es una concesión; esto es un muro.
...
ObjectStack está construido para el comprador con un detonante. Es **self-hosteable**: el runtime vive donde tus datos y reguladores lo exigen, que es lo único que Power Platform estructuralmente no puede ofrecer. Se **conecta al CRM/ERP/BD que ya tienes** en lugar de obligarte a migrar tu negocio a Dataverse y alquilártelo de vuelta por gigabyte, así que no hay rehén ni proyecto de re-plataformización para irte. El núcleo de la app son **metadatos abiertos y legibles**, así que un cambio de IA estilo Copilot es un **diff revisable que un humano aprueba antes de que se publique**, no config gestionada opaca. Y como posees el runtime, la productividad de IA deja de ser un contador por mensaje: el coste marginal de más éxito es tu infraestructura, no un impuesto.
