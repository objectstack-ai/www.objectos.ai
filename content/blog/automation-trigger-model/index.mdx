...
ObjectStack declares it. `runAs` defaults to `'user'` — the triggering user, with [row-level security](/en/glossary/row-level-security/) respected. A run whose trigger supplied **no** user has nothing to scope to, so its data operations are refused rather than silently elevated:
