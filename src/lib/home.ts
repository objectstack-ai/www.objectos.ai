...
const zhHans: HomeCopy = {
  ...
  aiCopy: 'ObjectOS 把对象、字段、流程、权限和动作变成 Agent 可读写的声明式元数据，并通过受治理工具访问。开源的 ObjectStack 采用自带 AI（BYO-AI）：你的编码 Agent 以源码方式编写元数据——随附的 CRM 是 31 个文件、1792 行，整个业务系统装得进 Agent 的上下文；你审阅 diff，任意 MCP 客户端即可问询数据。底下重复的部分全由运行时提供：建表、API、界面、权限校验、审计。界面内 Build 与 Ask 助手在云端与企业版提供。',
  aiItems: [
...
const ko: HomeCopy = {
  ...
  aiCopy: 'ObjectOS는 객체, 필드, 워크플로, 권한, 작업을 에이전트가 거버넌스 도구로 읽고 업데이트할 수 있는 선언형 메타데이터로 만듭니다. 오픈소스 ObjectStack에서는 당신의 AI를 가져옵니다. 코딩 에이전트가 메타데이터를 소스 파일로 작성하며, 이는 생성된 앱 코드베이스의 아주 작은 일부인 경우가 많습니다. 당신은 diff를 검토하고 모든 MCP 클라이언트가 데이터를 조회할 수 있습니다. 함께 제공되는 CRM은 31개 파일 1,792줄로, 전체 비즈니스 시스템이 에이전트의 컨텍스트에 들어갑니다. 그 아래 반복되는 부분 — 테이블, API, UI, 권한 검사, 감사 — 은 모두 런타임이 제공합니다. 제품 내 Build와 Ask 어시스턴트는 Cloud와 Enterprise에서 실행됩니다.',
  aiItems: [
