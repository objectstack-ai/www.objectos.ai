...
const zhHans: SecurityCopy = {
  ...
  description: '了解 ObjectOS 如何在自托管环境中保护数据驻留、权限边界、AI Agent 操作和审计证据。',
  eyebrow: '安全与治理',
  hero: ['数据留在你的网络里，', 'AI 在权限边界内工作'],
  lead: 'ObjectOS 是运行在你基础设施中的自托管运行时。业务数据、身份、审计日志、文件和密钥由你控制；AI Agent 只能通过受控工具访问对象，并继承登录用户已有的权限。',
  metrics: [
    { value: '0', label: '默认外发遥测' },
    { value: '5 层', label: '身份到字段的访问模型' },
    { value: 'HITL', label: 'AI 变更审批' },
  ],
  residencyTitle: '数据驻留',
  residencyLead: 'ObjectOS 不要求你把业务数据搬到我们的云上。运行时连接你的数据库、存储和身份系统，数据驻留边界由你的部署方式决定。',
  ...
  aiLead: 'ObjectOS 不把模型当作超级管理员。AI 通过声明式工具工作，工具输入经过校验，写操作进入审批队列，并受同一套权限系统约束。',
  aiSteps: [
  ...
  deploymentLead: 'ObjectOS 的默认形态适合企业内网部署：一个 Node.js 运行时、你的数据库、你的密钥和你的入口层。需要更严格边界时，也可以走文件制品和离线镜像。',
  deployments: [
  ...
  faqs: [
    ...
    { question: '谁负责 TLS、数据库加密和密钥管理？', answer: '这些属于你的部署责任。ObjectOS 负责运行时权限、审计、API key 哈希存储和设置加密；TLS、数据库静态加密、备份和密钥注入由你的基础设施负责。' },
    { question: '它适合合规行业吗？', answer: 'ObjectOS 提供访问控制、审计、数据驻留和隔离部署这些技术原语。具体认证取决于你的运行中部署，而不是单个二进制。' },
  ],
  ctaKicker: '下一步',
  ctaTitle: '把安全评审前置，而不是等到试点后补。',
  ctaCopy: '先确认数据驻留、权限模型、审计证据和部署边界，再把 AI Agent 接到真实业务数据上。',
  ctaPrimary: '阅读安全文档',
...
const en: SecurityCopy = {
  ...
  description: 'Learn how ObjectOS protects data residency, permissions, AI agent actions, and audit evidence in self-hosted environments.',
  eyebrow: 'Security and governance',
  hero: ['Keep data in your network.', 'Let AI work inside permissions.'],
  lead: 'ObjectOS is a self-hosted runtime that runs on your infrastructure. Business records, identities, audit logs, files, and secrets stay under your control; AI agents access objects through governed tools and inherit the permissions of the signed-in user.',
  metrics: [
    { value: '0', label: 'default outbound telemetry' },
    { value: '5 layers', label: 'identity-to-field access model' },
    { value: 'HITL', label: 'approval for AI changes' },
  ],
  ...
  aiLead: 'ObjectOS does not treat the model as an administrator. AI works through declarative tools, tool input is validated, mutating work goes through approval, and every call is constrained by the same permission system.',
  aiSteps: [
  ...
  deploymentLead: 'The default shape is enterprise-network friendly: one Node.js runtime, your database, your secrets, and your ingress. Stricter environments can use file-backed artifacts and offline images.',
  deployments: [
  ...
  ctaKicker: 'Next step',
  ctaTitle: 'Put security review first, not after the pilot.',
  ctaCopy: 'Confirm data residency, permission boundaries, audit evidence, and deployment shape before connecting AI agents to real business data.',
...
const ja: SecurityCopy = {
  ...
  lead: 'ObjectOS は自社インフラで動くセルフホスト型ランタイムです。業務データ、ID、監査ログ、ファイル、シークレットは自社管理のまま。AI エージェントは管理されたツール経由でオブジェクトにアクセスし、ログインユーザーの権限を継承します。',
  metrics: [
  ...
  residencyLead: 'ObjectOS は業務データを当社クラウドへ移すことを求めません。ランタイムは自社のデータベース、ストレージ、ID 基盤へ接続します。',
...
const de: SecurityCopy = {
  ...
  lead: 'ObjectOS ist eine selbst gehostete Runtime auf Ihrer Infrastruktur. Geschäftsdaten, Identitäten, Audit-Logs, Dateien und Secrets bleiben unter Ihrer Kontrolle; AI Agents greifen über kontrollierte Tools auf Objekte zu und erben die Rechte des angemeldeten Nutzers.',
  residencyTitle: 'Datenresidenz',
  residencyLead: 'ObjectOS verlangt nicht, Geschäftsdaten in unsere Cloud zu verschieben. Die Runtime verbindet sich mit Ihren Datenbanken, Speichern und Identitätssystemen.',
...
const es: SecurityCopy = {
  ...
  lead: 'ObjectOS es un runtime autohospedado que corre en tu infraestructura. Registros de negocio, identidades, auditoría, archivos y secretos siguen bajo tu control; los agentes de AI acceden a objetos mediante herramientas gobernadas y heredan permisos del usuario conectado.',
  residencyTitle: 'Residencia de datos',
  residencyLead: 'ObjectOS no exige mover datos de negocio a nuestra nube. El runtime se conecta a tus bases de datos, almacenamiento e identidad.',
...
const fr: SecurityCopy = {
  ...
  lead: 'ObjectOS est un runtime auto-hébergé sur votre infrastructure. Données métier, identités, journaux d’audit, fichiers et secrets restent sous votre contrôle ; les agents AI accèdent aux objets via des outils gouvernés et héritent des permissions de l’utilisateur connecté.',
  residencyTitle: 'Résidence des données',
  residencyLead: 'ObjectOS n’exige pas de déplacer vos données métier dans notre cloud. Le runtime se connecte à vos bases de données, stockages et systèmes d’identité.',
...
const ko: SecurityCopy = {
  ...
  lead: 'ObjectOS는 사용자의 인프라에서 실행되는 셀프 호스팅 런타임입니다. 비즈니스 기록, ID, 감사 로그, 파일과 시크릿은 사용자가 통제하며, AI 에이전트는 관리되는 도구로 객체에 접근하고 로그인한 사용자의 권한을 상속합니다.',
  residencyTitle: '데이터 레지던시',
  residencyLead: 'ObjectOS는 비즈니스 데이터를 당사 클라우드로 옮기라고 요구하지 않습니다. 런타임은 사용자의 데이터베이스, 스토리지, ID 시스템에 연결됩니다.',
