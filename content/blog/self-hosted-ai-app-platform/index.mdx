---
title: "Self-Hosted AI Application Platforms: Why the Runtime Belongs to You"
...
**The short version:** What you should self-host first is not necessarily the model. It is the runtime that holds objects, permissions, tools, approvals, and audit. Models can come from outside; the layer that decides who can read what, do what, and what evidence is kept belongs on your own infrastructure.
...
![Self-hosted AI application runtime boundary](./self-hosted-runtime-en.svg)
...
The key is that the model should not connect directly to the database or hold business-system credentials. It should receive only the minimum approved context through the open-source ObjectStack Runtime. ObjectOS is the optional commercial production platform and operated experience for the same ObjectStack application; it does not replace that runtime boundary.
...
## ObjectStack Runtime and the Optional ObjectOS Layer

The open-source ObjectStack Runtime does not start by moving enterprise data into a new cloud system so AI can work on top of it.

It is the self-hosted business operating layer deployed inside enterprise infrastructure: connect existing systems, model them as objects, and unify permissions, workflows, APIs, audit, and agent tools.

ObjectOS is the optional commercial production platform and operated experience for that same ObjectStack application. It adds browser-based AI building, deployment, support, and operations without changing the open definition or the ObjectStack Runtime underneath.
