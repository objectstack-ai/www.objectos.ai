...
- **Outbound, you own the budget.** The platform retries on one fixed cadence, so you know how long it keeps trying and exactly when it gives up — and you own what happens after it does. You also own the consequence: every retry is another chance to apply the same change twice at the far end. What the platform hands you is an anchor rather than a dial: every attempt at one delivery carries that delivery's id in an `X-Objectstack-Delivery` header, so a receiver that deduplicates on it will not post the same change twice.
...
Here is the number to hold on to. When an outbound delivery goes through the durable outbox, the retry schedule is **fixed at seven backoff steps — roughly 1 second, 10 seconds, 1 minute, 10 minutes, 1 hour, 6 hours, then 24 hours, each with ±20% jitter — after which the row is dead-lettered** rather than retried forever. End to end that is about 31 hours from the first failure to the dead-letter row.

Read that number as an operational fact, not a trivia item. It means a delivery can take more than a day to fail for good, so "the other side never got it" does not announce itself while anyone is still looking at the run — and it means a dead-lettered row nobody is assigned to is an incident with a paper trail and no owner.
...
| Messaging outbox | Delivery of outbound webhooks and durable HTTP calls. | The fixed seven-step schedule above, then a dead-letter row. |
