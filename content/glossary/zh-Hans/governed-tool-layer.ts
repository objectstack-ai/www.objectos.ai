...
const term = {
  ...
  explanation: [
    ...
    'ObjectStack 把这一层从元数据里生成出来。一个动作只有在它自己的定义里主动选择开放，才会成为 Agent 可调用的工具——`ai.exposed` 默认为 false，什么都不写的动作就是没有开放；而已开放的动作，走的是与 REST 路由同一道权限闸门、同一个动作执行器，而不是另开一条并行通道。有一条诚实的边界对任何实现都成立，包括这一个：受治理工具层约束的是影响半径，不是判断力。它能阻止 Agent 去做它无权做的事，却拦不住一次「在权限之内但本不该做」的操作。那个缺口属于评测、高风险动作的审批阈值和流程设计。',
  ],
