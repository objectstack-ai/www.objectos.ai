...
| Self-hosted runtime | ObjectStack runtime; ObjectOS for managed operations or private deployment | Grows with infrastructure, basically decoupled from usage | Good — double the usage and the curve barely moves | Your own infrastructure |
...
The core is one distinction: **ObjectStack is the open execution engine and runtime for the business definition; ObjectOS charges for the production experience of operating that same ObjectStack app, not for a second engine or each runtime call.** That experience includes Build and Ask, team controls, managed or private deployment, and support. From this:

- **Cost decoupled from usage.** The ObjectStack runtime executes the business definition on your infrastructure, and however many times the agent calls a governed tool, the runtime is not billed per call — cost is determined by infrastructure: predictable and plannable. ObjectOS pricing covers production operations around the app, not a meter on runtime calls.
...
- **No lock-in.** The business definition is metadata in your repository under an open protocol (Apache 2.0): diffable, migratable. What you buy from ObjectOS is the production experience of operating the same open ObjectStack app, not a proprietary runtime you cannot leave.
