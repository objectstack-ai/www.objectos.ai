...
const zhHans: HomeCopy = {
  ...
  positioningCopy: '企业 AI 不需要又一次重建，也不需要更多不可审的生成代码。它需要一种 Agent 能写、人能审、运行时能治理的紧凑目标格式，把旧系统、新应用和 AI Agent 接在一起。你的对象、权限、流程就是你的开放业务本体——它应该是你拥有的、带版本的文件，而不是锁在别人平台里的资产。',
...
const ja: HomeCopy = {
  ...
  positioningCopy: 'エンタープライズ AI に必要なのは、また一つの再構築プロジェクトでも、レビュー不能な生成コードの山でもありません。エージェントが書き、人がレビューでき、ランタイムが統制するコンパクトなターゲット形式です。オブジェクト、権限、ワークフローはあなたのオープンなビジネスオントロジーです — 誰かのプラットフォームに閉じ込められた資産ではなく、あなたが所有するバージョン管理されたファイルであるべきです。',
...
const ko: HomeCopy = {
  ...
  positioningCopy: '엔터프라이즈 AI에 필요한 것은 또 하나의 재구축 프로젝트도, 검토하기 어려운 생성 코드 더미도 아닙니다. 에이전트가 작성하고 사람이 검토하며 런타임이 기존 시스템, 새 애플리케이션, AI 에이전트 전반에서 거버넌스를 유지하는 압축된 대상 형식입니다. 객체, 권한, 워크플로는 당신의 열린 비즈니스 온톨로지입니다 — 남의 플랫폼에 갇힌 자산이 아니라 당신이 소유한 버전 관리되는 파일이어야 합니다.',
