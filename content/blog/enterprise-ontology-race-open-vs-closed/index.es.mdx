---
title: "Los gigantes corren por construir la ontología empresarial, pero todas son cerradas: ¿será la capa semántica abierta el TCP/IP de la era de la IA?"
description: "El bloqueo de un solo proveedor es el problema viejo; tres gigantes construyendo ontologías cerradas a la vez crean uno nuevo y peor: tu \"cliente\" queda partido en tres y ningún agent puede responder al conjunto. Lo que de verdad falta no es \"más apertura\", sino una capa de definición neutral que no pertenezca a nadie."
...
Hace un año, "capa semántica de negocio / Ontology" era prácticamente vocabulario exclusivo de Palantir. Este año, Gartner posicionó en su cumbre de datos y analítica la capa semántica y los grafos de conocimiento directamente como infraestructura de la IA agéntica; las previsiones del mercado de grafos de conocimiento empresarial van de unos 1.900 millones de dólares hoy hasta casi 10.000 millones en 2032. Y los jugadores ya están todos en la mesa: Microsoft impulsa Fabric IQ, Google saca a escena su grafo de conocimiento empresarial y Palantir vende Foundry mientras se ata al cómputo de NVIDIA.

¿Por qué precisamente ahora? Porque los agents destaparon el agujero. Las tuberías de datos de la última década solo se preocupaban de "a dónde fluye el dato", no tanto de "qué significa este dato"; al fin y al cabo, al final siempre había una persona que cubría. El agent no tiene esa persona que cubra: enfrentado a decenas de tablas y a campos de "cliente" con criterios dispares, o responde mal (como con el Grupo H) o no se atreve a responder. Esas cifras llamativas que circulan por el sector —el 88% de los pilotos de agent no llegan a producción— mueren en buena parte por la ausencia de esta capa.
...
Supongamos que Yuanfeng hubiera construido "cliente" como una definición neutral desde el principio: con una sola declaración podría recoger los datos dispersos en tres sistemas dentro del mismo objeto.
...
export const Customer = ObjectSchema.create({
  ...
  label: 'Cliente',
  sources: [
    Source.fabric('Account'),              // los pedidos y equipos están aquí
    Source.salesforce('Account'),          // los registros comerciales de la adquisición están aquí
    Source.database('eu.customers'),       // los datos europeos aislados por cumplimiento están aquí
  ],
  identity: ['tax_id'],                    // criterio unificado para determinar "el mismo cliente"
});
```
...
os deploy --self-hosted   # la misma definición, corriendo en tu propia infraestructura
